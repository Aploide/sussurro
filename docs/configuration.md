...
`models.llm.extended_prompt: true` switches to a richer instruction set that
also formats dictated enumerations as lists and adds paragraph breaks. The
...
general instruct model in `models.llm.path`.
